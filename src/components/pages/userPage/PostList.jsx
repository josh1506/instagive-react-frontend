...
onClick={() => props.history.push('/user/post-create')}
      ...
      <div className='postCardContainer shadow-container'>
         ...
         {props.post.filter(post => post.status === 'Approved').length === 0 ? <h2>You have no approved post</h2> : props.post.map((post)  => post.status === 'Approved' ?  (
...
{props.post.filter(post => post.status === 'Pending').length === 0  ? <h2>You have no pending post</h2> : props.post.map((post)  => post.status === 'Pending' ?  (
...
{props.post.filter(post => post.status === 'Rejected').length === 0 ? <h2>You have no rejected/deleted post</h2> : props.post.map((post)  => post.status === 'Rejected' ?  (
